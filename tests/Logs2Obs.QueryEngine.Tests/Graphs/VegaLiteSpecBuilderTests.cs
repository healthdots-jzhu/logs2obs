...
public class VegaLiteSpecBuilderTests
{
    [Fact]
    public void Build_LineChart_ReturnsValidSpec() =>
        AssertSpec(GraphType.LineChart);

    [Fact]
    public void Build_BarChart_ReturnsValidSpec() =>
        AssertSpec(GraphType.BarChart);

    [Fact]
    public void Build_AreaChart_ReturnsValidSpec() =>
        AssertSpec(GraphType.AreaChart);

    [Fact]
    public void Build_PieChart_ReturnsValidSpec() =>
        AssertSpec(GraphType.PieChart);

    [Fact]
    public void Build_HeatMap_ReturnsValidSpec() =>
        AssertSpec(GraphType.HeatMap);

    [Fact]
    public void Build_Scatter_ReturnsValidSpec() =>
        AssertSpec(GraphType.Scatter);

    [Fact]
    public void Build_Stat_ReturnsValidSpec() =>
        AssertSpec(GraphType.Stat);

    [Fact]
    public void Build_Gauge_ReturnsValidSpec() =>
        AssertSpec(GraphType.Gauge);

    [Fact]
    public void Build_StackedAreaChart_ReturnsValidSpec() =>
        AssertSpec(GraphType.StackedAreaChart);

    private static void AssertSpec(GraphType graphType)
    {
        var spec = VegaLiteSpecBuilder.Build(graphType, BuildSchema(graphType), BuildResults(graphType));
