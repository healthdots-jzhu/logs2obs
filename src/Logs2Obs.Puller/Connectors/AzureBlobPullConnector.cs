...
public sealed class AzureBlobPullConnector : IPullConnector
{
    private readonly IObjectStore _objectStore;
    private readonly IMetadataStore _metadataStore;
    private readonly ILogger<AzureBlobPullConnector> _logger;
    private readonly ResiliencePipeline _resilience;

    public AzureBlobPullConnector(
        IObjectStore objectStore,
        IMetadataStore metadataStore,
        ILogger<AzureBlobPullConnector> logger)
    {
    ...
    {
        var container = config.ConnectorConfig.GetValueOrDefault("container") ?? throw new InvalidOperationException("Missing container config");
        var prefix = config.ConnectorConfig.GetValueOrDefault("prefix") ?? string.Empty;
        var objectPrefix = BuildObjectPrefix(container, prefix);
    ...
    private static string BuildObjectPrefix(string container, string prefix)
    {
        var sanitizedContainer = container.TrimEnd('/');
        if (string.IsNullOrWhiteSpace(prefix))
        {
            return sanitizedContainer;
        }
        return $"{sanitizedContainer}/{prefix.TrimStart('/')}";
    }
}
